...
namespace TalesTaleWorlds
{
   ...
   public class IntegratedLoaderSubModule : MBSubModuleBase
   {
      ...
      private static void LoadTexturesIntoRuntime()
      {
         ...
         foreach (FileInfo imgFile in GameData.Instance.StoryContext.StoryImagesFolder.GetFiles("*.png"))
         {
            var texture = Texture.LoadTextureFromPath(imgFile.FullName, imgFile.DirectoryName);
            texture.PreloadTexture();
            var texture2D = new TaleWorlds.TwoDimension.Texture(new EngineTexture(texture));

            Textures.TextureList.Add(Path.GetFileNameWithoutExtension(imgFile.Name), texture2D);
            GameData.Instance.StoryContext.StoryImageFiles.Add(imgFile);
         }
      }
